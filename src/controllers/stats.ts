...
export const getDashboardStats = TryCatch(async (req, res, next) => {
	...
	if (myCache.has(key)) {
		...
		const orderMonthCounts = getChartData({ length: 6, today, docArr: lastSixMonthsOrders });

		const orderMonthlyRevenue = getChartData({ length: 6, today, docArr: lastSixMonthsOrders, property: 'total' });
...
export const getBarCharts = TryCatch(async (req, res, next) => {
	...
	if (myCache.has(key)) {
		...
		const sixMonthsProductsPromise = Order.find({
			createdAt: {
				$gte: sixMonthsAgo,
				$lte: today,
			},
		...
		const sixMonthsUsersPromise = Order.find({
			createdAt: {
				$gte: sixMonthsAgo,
				$lte: today,
			},
		...
		const [products, users, orders] = await Promise.all([sixMonthsProductsPromise, sixMonthsUsersPromise, tweleveMonthsOrdersPromise]);
